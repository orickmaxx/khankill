...
const repoPath = `https://raw.githubusercontent.com/orickmaxx/khankill/refs/heads/${isDev ? "dev/" : "main/"}`;
...
/* Misc Styles */
document.head.appendChild(Object.assign(document.createElement("style"),{innerHTML:"@font-face{font-family:'MuseoSans';src:url('https://corsproxy.io/?url=https://r2.e-z.host/4d0a0bea-60f8-44d6-9e74-3032a64a9f32/ynddewua.ttf')format('truetype')} @keyframes chromaShift { 0% { color: #ff0000; } 20% { color: #0000ff; } 40% { color: #ff00ff; } 60% { color: #ffff00; } 80% { color: #00ffff; } 100% { color: #ff0000; } }"}));
...
async function showSplashScreen() { 
    ...
    splashScreen.innerHTML = '<span style="color:white;">KHANKILL</span><span style="animation: chromaShift 5s infinite;">.SPACE</span>'; 
...
async function loadScript(url, label) { 
    try {
        ...
        sendToast(`🚨 Failed to load ${label}: ${e.message}`, 5000);
        console.error(e);
    }
}
...
function setupMenu() {
    Promise.all([
        ...
        isDev ? loadScript(repoPath+'visuals/devTab.js', 'devTab') : Promise.resolve()
    ]).catch(e => console.error(`Failed to load menu scripts: ${e}`));
}
...
function setupMain() {
    ...
    ]).catch(e => console.error(`Failed to load main scripts: ${e}`));
}

/* Inject */
if (!/^https?:\/\/([a-z0-9-]+\.)?khanacademy\.org/.test(window.location.href)) { 
    alert("❌ Khankill Failed to Injected!\n\nVocê precisa executar o Khankill no site do Khan Academy! (https://pt.khanacademy.org/)"); 
    window.location.href = "https://pt.khanacademy.org/"; 
}

showSplashScreen();

// Fallback timeout to hide splash screen after 10 seconds if scripts fail to load
...
Promise.all([
    ...
    loadCss('https://cdn.jsdelivr.net/npm/toastify-js/src/toastify.min.css').catch(e => console.error(`Failed to load Toastify CSS: ${e}`)),
    ...
    try {
        ...
        sendToast(`🚨 Failed to fetch user profile: ${e.message}`, 5000);
        console.error(e);
    }
    
    sendToast("🌿 Khankill injetado com sucesso!");
    
    playAudio('https://r2.e-z.host/4d0a0bea-60f8-44d6-9e74-3032a64a9f32/gcelzszy.wav').catch(e => console.error(`Failed to play injection audio: ${e}`));
    ...
    console.error(`Failed to load initial scripts: ${e}`);
    sendToast(`🚨 Failed to initialize Khankill: ${e.message}`, 5000);
    hideSplashScreen();
});
